...
export class VideoTranslationComponent {
  ...
  submit() {
    ...
    const existingTrack = this.lstTrack.find(
      (track: { srclang: string }) => track.srclang === this.language.value,
    );

    if (!existingTrack) {
      const selectedLanguage = this.lstLanguages.find(
        (lang) => lang.LanguageCulture === this.language.value,
      );
      ...
      this._http
        .post(
          'http://localhost:5176/api/Video/TranslateTranscription?targetLanguage=' +
            this.language.value,
          formData,
          { responseType: 'blob' },
        )
  ...
  onTimeUpdate() {
    if (this.videoElement && this.videoElement.nativeElement) {
      ...
      this.currentTimestamp =
        this.convertToTimestamp(currentTime).split('.')[0];
