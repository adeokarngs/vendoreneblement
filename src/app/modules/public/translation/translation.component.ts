  ...
  submit() {
    if (this.tForm.valid) {
      if (this.file) {
        ...
        this._http.post("http://localhost:5176/api/Video/UploadAndTranslate?targetLanguage=" + this.tForm.value?.language, form)
          .subscribe((resp: any) => {
    ...
    this.tForm.controls['text'].valueChanges
      ...
      .subscribe(resp => {
        ...
        if (this.tForm.get("text").valid) {
          ...
          this._http.post("http://localhost:5176/api/Video/translate", payload)
            .subscribe((resp: any) => {
              this.translatedText += '\n' + resp['translatedText']
